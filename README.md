...
- **`orders.served_at`** (timestamp) and **`orders.cancelled_by`** (text) were added to support two later features: a simulated delivery timeline on the customer side (Preparing → Delivering → Served, spread over the 2 minutes after the waiter marks an order served — see `lib/deliveryStage.ts`), and order cancellation from either the customer or waiter view, allowed only while an order is still `Pending`.
